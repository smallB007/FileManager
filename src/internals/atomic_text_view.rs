...
/// Provides access to the content of a [`AtomicTextView`].
///
/// [`AtomicTextView`]: struct.AtomicTextView.html
///
...
///
/// ```rust
/// # use cursive_core::views::{AtomicTextView, TextContent};
/// let mut content = TextContent::new("content");
/// let view = AtomicTextView::new_with_content(content.clone());
///
...
impl TextContent {
    ...
    /// Append `content` to the end of a `AtomicTextView`.
...
/// Internel representation of the content for a `AtomicTextView`.
///
...
impl TextContentInner {
    ...
    fn get_content(content: &Arc<Mutex<TextContentInner>>) -> TextContentRef {
        ...
        let _handle = OwningHandle::new_with_fn(arc_ref, |mutex| unsafe { (*mutex).lock().unwrap() });
...
/// # use cursive_core::views::AtomicTextView;
/// let mut siv = Cursive::new();
///
/// siv.add_layer(AtomicTextView::new("Hello world!"));
/// ```
pub struct AtomicTextView {
    // content: String,
    content: TextContent,
    rows: Vec<Row>,
...
impl AtomicTextView {
    /// Creates a new AtomicTextView with the given content.
    pub fn new<S>(content: S) -> Self
    where
        S: Into<StyledString>,
    {
    ...
    /// Creates a new AtomicTextView using the given `TextContent`.
    ///
    ...
    /// Creates a new empty `AtomicTextView`.
    pub fn empty() -> Self {
        AtomicTextView::new("")
    }
...
impl View for AtomicTextView {
    ...
    fn on_event(&mut self, event: Event) -> EventResult {
        ...
        let width = 1000; //self.content.width();
