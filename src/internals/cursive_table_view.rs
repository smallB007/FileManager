...
impl<T: TableViewItem<H>, H: Eq + Hash + Copy + Clone + 'static> TableView<T, H> {
    ...
    pub fn insert_column<S: Into<String>, C: FnOnce(TableColumn<H>) -> TableColumn<H>>(
        ...
        self.columns.insert(i, callback(TableColumn::new(column, title.into())));
    ...
    pub fn clear_selected_item(&mut self, inx: usize) {
        self.rows_selected.remove(&inx);
    }
    //--artie
...
impl<T: TableViewItem<H>, H: Eq + Hash + Copy + Clone + 'static> TableView<T, H> {
    fn draw_columns<C: Fn(&Printer, &TableColumn<H>)>(&self, printer: &Printer, sep: &str, callback: C) {
    ...
    fn on_inner_event(&mut self, event: Event) -> EventResult {
        ...
        match event {
            ...
            } if !self.is_empty() && position.checked_sub(offset).map_or(false, |p| p.y == self.focus) => {
                self.column_cancel();
                return self.on_submit_event();
            }
            Event::Mouse {
...
mod tests {
    ...
    fn setup_test_table() -> TableView<SimpleItem, SimpleColumn> {
        TableView::<SimpleItem, SimpleColumn>::new().column(SimpleColumn::Name, "Name", |c| c.width_percent(20))
    }
